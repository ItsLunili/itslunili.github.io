...
document.addEventListener("DOMContentLoaded", () => {
    const invite_link = 'https://discord.com/oauth2/authorize?client_id=1524817994847617065&permissions=4503874508434944&integration_type=0&scope=bot';

    document.querySelectorAll('.bot-invite-link').forEach(el => {
        el.href = invite_link;
    });
});
...
document.querySelectorAll('.message').forEach(msg => {
    msg.innerHTML = msg.innerHTML.replace(
        /#(\w+)/g,
        '<span class="channel-mention">#$1</span>'
    );
});
...
pricingToggle.addEventListener('click', () => {
    ...
    priceValues.forEach(el => {
        ...
        if (el.classList.contains('free-label')) return;
